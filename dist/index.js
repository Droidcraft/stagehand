...
function buildBackendIdMaps(page) {
  return __async(this, null, function* () {
    yield page.enableCDP("DOM");
    try {
      const { root } = yield page.sendCDP("DOM.getDocument", {
        depth: -1,
        pierce: true
      });
      const tagNameMap = {};
      ...
      const walk = (node, path4) => {
        ...
        if (node.backendNodeId) {
          ...
          xpathMap[node.backendNodeId] = path4;
        }
        if (!((_a15 = node.children) == null ? void 0 : _a15.length)) return;
        const counters = {};
        for (const child of node.children) {
          const name14 = String(child.nodeName).toLowerCase();
          const counterKey = `${child.nodeType}:${name14}`;
          const idx = counters[counterKey] = ((_b = counters[counterKey]) != null ? _b : 0) + 1;
          const seg = child.nodeType === 3 ? `text()[${idx}]` : child.nodeType === 8 ? `comment()[${idx}]` : `${name14}[${idx}]`;
          walk(child, `${path4}/${seg}`);
        }
      };
      walk(root, "");
      return { tagNameMap, xpathMap };
    } finally {
      yield page.disableCDP("DOM");
    }
  });
}
function cleanStructuralNodes(node, tagNameMap, logger) {
  return __async(this, null, function* () {
    ...
    const cleanedChildrenPromises = node.children.map(
      (child) => cleanStructuralNodes(child, tagNameMap, logger)
    );
    ...
    if ((node.role === "generic" || node.role === "none") && node.backendDOMNodeId !== void 0) {
...
function buildHierarchicalTree(nodes, tagNameMap, logger, xpathMap) {
  return __async(this, null, function* () {
    ...
    const cleanedTreePromises = rootNodes.map(
      (node) => cleanStructuralNodes(node, tagNameMap, logger)
    );
...
function getAccessibilityTree(page, logger, selector) {
  return __async(this, null, function* () {
    var _a15;
    const { tagNameMap, xpathMap } = yield buildBackendIdMaps(page);
